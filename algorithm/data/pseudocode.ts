export const pseudocodes: Record<string, string[]> = {
  ...
  'strongly-connected-components': [
    ...
    '  finishOrder ← []',
    '  DFS1(G)로 각 정점의 완료 순서 계산',
    '  G^R ← transpose(G)',
    '  모든 정점의 방문 표시를 false로 초기화',
    '  for (finishOrder를 뒤에서부터 순회)',
    '    if (해당 정점이 미방문)',
    '      component ← DFS2(G^R, 정점)',
    '      component를 강연결 성분으로 출력',
    '',
    'DFS1(G, v)',
    '  visited[v] ← TRUE',
    '  for (v의 인접 정점 w)',
    '    if (visited[w] == FALSE) DFS1(G, w)',
    '  finishOrder에 v 추가',
    '',
    'DFS2(G^R, v)',
    '  visited[v] ← TRUE',
    '  component에 v 추가',
    '  for (G^R에서 v의 인접 정점 w)',
    '    if (visited[w] == FALSE) DFS2(G^R, w)',
  ],
  kruskal: [
    'Kruskal(G)',
    '  T = ∅;',
