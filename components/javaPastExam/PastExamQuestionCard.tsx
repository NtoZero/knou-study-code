...
import JavaCodePanel from "./JavaCodePanel";
import type { JavaChoiceKey, JavaPastExamQuestion } from "./types";

type Props = {
  question: JavaPastExamQuestion;
  selected?: JavaChoiceKey;
  answerRevealed?: boolean;
  explanationExpanded?: boolean;
  compact?: boolean;
  onSelect: (questionId: string, choice: JavaChoiceKey) => void;
    ...
    >
      <div className="border-b border-gray-100 px-4 py-3 dark:border-gray-800 sm:px-5">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-md bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-900 dark:bg-amber-950 dark:text-amber-100">
      ...
      <div className={`space-y-4 p-4 sm:p-5 ${compact ? "text-sm" : ""}`}>
        <p className="whitespace-pre-wrap break-keep rounded-md border border-gray-200 bg-gray-50 p-3 text-sm leading-7 text-gray-900 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-100">
          {question.prompt}
        </p>

        {question.codeBlocks?.map((block) => (
          <JavaCodePanel key={`${question.id}-${block.title}`} title={block.title} code={block.code} />
        ))}
          ...
          {question.choices.map((choice) => {
            ...
            const state = !answerRevealed
              ? active
                ...
                : "border-gray-200 bg-white hover:border-amber-300 dark:border-gray-700 dark:bg-gray-950"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {answerRevealed && (
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-900 hover:bg-amber-100 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-100 dark:hover:bg-amber-950"
            >
        ...
        <AnimatePresence initial={false}>
          {answerRevealed && explanationExpanded && (
            ...
            >
              <div className="space-y-3 rounded-lg border border-amber-100 bg-amber-50/70 p-4 dark:border-amber-900 dark:bg-amber-950/20">
                {question.choices.map((choice) => {
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {question.lectureRefs.map((ref) => (
                    <Link
                      ...
                      href={ref.href}
                      className="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-white px-3 py-1.5 text-xs font-semibold text-amber-900 hover:bg-amber-100 dark:border-amber-800 dark:bg-gray-900 dark:text-amber-100 dark:hover:bg-amber-950"
                    >
                      <BookOpen size={13} />
                      {ref.label}
                    </Link>
