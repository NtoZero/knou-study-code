...
type Props = {
  question: NetworkReconstructedQuestion;
  selected?: NetworkChoiceKey;
  answerRevealed?: boolean;
  explanationExpanded?: boolean;
  onSelect: (questionId: string, choice: NetworkChoiceKey) => void;
...
const choiceStyle: Record<ChoiceState, string> = {
  idle:
    "border-gray-200 bg-white text-gray-800 hover:border-emerald-300 hover:bg-emerald-50 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:hover:border-emerald-600 dark:hover:bg-emerald-950/30",
  selected:
...
export default function NetworkReconstructedQuestionCard({
  question,
  selected,
  answerRevealed,
  explanationExpanded,
  ...
  const answerVisible = Boolean(answerRevealed);
  const explanationOpen = Boolean(explanationExpanded);
    ...
    >
      <div className="border-b border-gray-100 px-4 py-3 dark:border-gray-800 sm:px-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-md bg-gray-100 px-2.5 py-1 text-xs font-semibold text-gray-600 dark:bg-gray-900 dark:text-gray-300">
            {question.category}
          </span>
      ...
      <div className="space-y-4 p-4 sm:p-5">
        <div className="rounded-md border border-gray-200 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-900">
          <div className="mb-2 flex items-center gap-2 text-xs font-bold text-emerald-700 dark:text-emerald-200">
            <Network size={14} />
            {question.sourceLabel}
          </div>
          <p className="break-keep text-sm leading-7 text-gray-900 dark:text-gray-100">
            {question.prompt}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {answerVisible && (
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-800 transition-colors hover:bg-emerald-100 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-100 dark:hover:bg-emerald-950"
            >
        ...
        {answerVisible && (
          <div className="rounded-lg border border-emerald-100 bg-emerald-50/70 p-4 dark:border-emerald-900 dark:bg-emerald-950/20">
            <div className="flex flex-wrap items-center gap-2 text-sm font-bold text-emerald-900 dark:text-emerald-100">
        ...
        {answerVisible && explanationOpen && (
          <div className="space-y-4 rounded-lg border border-emerald-100 bg-emerald-50/70 p-4 dark:border-emerald-900 dark:bg-emerald-950/20">
            ...
            <div className="flex flex-wrap gap-2">
              {question.lectureRefs.map((ref) => (
                <Link
                  ...
                  href={ref.href}
                  className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-white px-3 py-1.5 text-xs font-semibold text-emerald-800 transition-colors hover:bg-emerald-100 dark:border-emerald-800 dark:bg-gray-900 dark:text-emerald-100 dark:hover:bg-emerald-950"
                >
                  <BookOpen size={13} />
                  {ref.label}
                </Link>
