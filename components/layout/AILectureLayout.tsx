...
export default function AILectureLayout({ lectureId, children }: Props) {
  const lec = aiLectures[lectureId - 1];
  const prev = lectureId > 1 ? aiLectures[lectureId - 2] : null;
  const next = lectureId < aiLectures.length ? aiLectures[lectureId] : null;

  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      {/* Breadcrumb */}
      <div className="mb-6 flex items-center gap-2 text-sm text-gray-500">
        <Link href="/ai" className="hover:text-gray-900 dark:hover:text-gray-200">
          인공지능
        </Link>
