...
import { algorithmPastExamQuestions } from "./data";

type Track = "기초·분석" | "정렬" | "탐색" | "그래프" | "DP·스트링" | "NP";

const trackByChapter = (chapter: AlgorithmChapterId): Track => {
  if (chapter === 1) return "기초·분석";
  if (chapter === 2) return "정렬";
  if (chapter === 3) return "탐색";
  if (chapter === 4) return "그래프";
  if (chapter === 7) return "NP";
  return "DP·스트링";
};

const trackStyles: Record<Track, string> = {
  "기초·분석": "border-blue-200 bg-blue-50 text-blue-800 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-100",
  "정렬": "border-amber-200 bg-amber-50 text-amber-900 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-100",
  "탐색": "border-cyan-200 bg-cyan-50 text-cyan-800 dark:border-cyan-900 dark:bg-cyan-950/30 dark:text-cyan-100",
  "그래프": "border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-100",
  "DP·스트링": "border-fuchsia-200 bg-fuchsia-50 text-fuchsia-800 dark:border-fuchsia-900 dark:bg-fuchsia-950/30 dark:text-fuchsia-100",
  "NP": "border-rose-200 bg-rose-50 text-rose-800 dark:border-rose-900 dark:bg-rose-950/30 dark:text-rose-100",
};

function heatClass(count: number, active: boolean) {
  const ring = active ? " ring-2 ring-gray-950 ring-offset-2 dark:ring-white" : "";
  if (count >= 12) return `border-emerald-700 bg-emerald-600 text-white${ring}`;
  if (count >= 8) return `border-cyan-600 bg-cyan-500 text-white${ring}`;
  if (count >= 4) return `border-amber-400 bg-amber-200 text-amber-950${ring}`;
...
export default function AlgorithmExamPrepMap() {
  const [activeId, setActiveId] = useState(4);

  const lectureStats = useMemo(
    () =>
      algorithmLectures.map((lecture) => {
        const questions = algorithmPastExamQuestions.filter((question) =>
        ...
        return {
          ...
          track: trackByChapter(lecture.chapter),
          topConcepts,
        };
      }),
    [],
  ...
  const highYield = lectureStats.filter((item) => item.count >= 8).length;
  const chapterFocus = algorithmChapterWeights
    .filter((chapter) => chapter.count >= 4)
    .map((chapter) => `${chapter.title} ${chapter.count}`);

  return (
    <section className="mb-8 rounded-xl border border-emerald-200 bg-white p-5 shadow-sm dark:border-emerald-900 dark:bg-gray-950">
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-1 text-xs font-bold text-white">
            <Target size={14} />
            빈출 개념 지도
          </div>
          ...
          <p className="mt-1 text-sm leading-6 text-gray-500">
            진한 강의는 실제 기출 연결 문항이 많은 영역입니다. 정렬·그래프·탐색은 반복 출제량이 커서 먼저 묶어 봅니다.
          </p>
        </div>

        <div className="grid grid-cols-3 gap-2 text-center text-xs">
          <div className="rounded-lg border border-gray-200 px-3 py-2 dark:border-gray-800">
            <div className="font-mono text-lg font-bold">{algorithmPastExamQuestions.length}</div>
      ...
      <div className="grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-5 dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="mt-4">
            <div className="mb-2 text-sm font-bold">장별 우선순위</div>
            <div className="flex flex-wrap gap-2">
              {chapterFocus.map((item) => (
                <span
                  key={item}
                  className="rounded-lg bg-white px-2.5 py-1 text-xs font-semibold text-gray-600 dark:bg-gray-950 dark:text-gray-300"
                >
                  {item}
                </span>
              ))}
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <Link
              href={`/algorithm/lecture/${active.id}`}
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:bg-gray-950 dark:text-emerald-200 dark:hover:bg-emerald-950/40"
            >
              <BarChart3 size={16} />
              기출분석
            </Link>
