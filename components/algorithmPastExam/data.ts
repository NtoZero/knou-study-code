...
const CHOICE_REASON_OVERRIDES: Partial<Record<string, Partial<Record<ChoiceKey, string>>>> = {
  "2017-1-q12": {
    "1": "오답 근거: 1은 피벗 15보다 작아 왼쪽 부분배열에 남는 값이다. 교재 Partition 함수에서 교차 후 피벗 A[0]와 교환되는 값은 Right가 멈춘 A[Right]인데, 이 입력에서는 Right가 11에서 멈춘다.",
    "2": "오답 근거: 7도 피벗 15보다 작아 Left가 지나가는 값이다. 마지막에 피벗과 맞바뀌는 A[Right] 위치의 값이 아니므로 피벗이 있던 칸에 들어가는 데이터가 아니다.",
    "3": "정답 근거: 15 1 7 11 22에서 Left는 22에서, Right는 11에서 멈춘 뒤 서로 교차한다. 교재 Partition 절차는 이때 피벗 15와 A[Right]를 교환하므로 원래 피벗 칸에는 11이 들어간다.",
    "4": "오답 근거: 22는 피벗보다 크거나 같은 값이라 Left가 멈추는 위치이며 오른쪽 부분배열에 남는다. 피벗과 교환되는 값은 Left 위치의 22가 아니라 Right 위치의 11이다.",
  },
  "2017-1-q24": {
    "1": "정답 근거: 원문 방문 순서가 1, 3으로 시작한다. DFS는 3에서 5로 깊게 내려간 뒤 5의 미방문 인접 정점 2를 방문하므로 빈칸 (b)는 2이다.",
    "2": "오답 근거: 4는 시작 정점 1의 다른 인접 정점이지만, 1→3→5→2 경로를 처리한 뒤 되돌아와 방문된다. 빈칸 (b)처럼 5 바로 다음에 들어갈 정점이 아니다.",
    "3": "오답 근거: 5는 3에서 바로 내려가는 정점이므로 방문 순서의 앞 빈칸 (a)에 해당한다. 문항은 그 다음 빈칸 (b)를 묻기 때문에 5를 고르면 한 단계 빠른 방문 정점을 고른 것이다.",
    "4": "오답 근거: 6은 4를 방문한 뒤 4의 자식으로 내려갈 때 등장한다. DFS 경로상 2보다 뒤에 방문되므로 밑줄 친 (b)에 들어갈 수 없다.",
  },
  "2017-1-q28": {
    "1": "정답 근거: D⁽⁰⁾에서 1→3은 2, 3→2는 -1이므로 플로이드 갱신 뒤 d₁₂는 min(4, 2+(-1))=1이 된다. 최종 빈칸 중 (a)=1이 가장 작다.",
    "2": "오답 근거: (b)는 최종 d₁₃ 값이며 직접 간선 1→3의 비용 2가 그대로 최단 거리로 남는다. (a)=1보다 크므로 가장 작은 빈칸이 아니다.",
    "3": "오답 근거: (c)는 d₄₂ 값이다. 4→1→3→2 경로의 비용이 3+2-1=4로 계산되어 (a)=1보다 크다.",
    "4": "오답 근거: (d)는 d₄₃ 값이다. 4→1→3 경로의 비용이 3+2=5이므로 네 빈칸 중 최솟값이 아니다.",
  },
  "2018-1-q25": {
    "1": "오답 근거: 80을 끝으로 보낸 뒤 루트에 올라온 10은 더 큰 자식 70과 먼저 교환되어야 한다. 루트가 60으로 남은 배열은 최대 힙 재구성의 첫 비교를 잘못 적용한 결과이다.",
    "2": "정답 근거: 초기 힙 80 60 70 40 20 30 50 10에서 80과 10을 교환한 뒤, 10을 70과 바꾸고 다시 50과 바꾸면 70 60 50 40 20 30 10 80이 된다.",
    "3": "오답 근거: 재힙화에서 10은 70과 교환된 뒤 50과 교환된다. 20과 30의 위치까지 바꾸는 선택지는 자식 인덱스 관계를 넘어선 추가 교환을 한 배열이다.",
    "4": "오답 근거: 원문 초기 배열에는 90이 없고 최댓값 80이 정렬 구간 끝으로 이동해야 한다. 90이 나타나는 배열은 주어진 힙 정렬 단계에서 만들 수 없다.",
  },
  "2018-1-q28": {
    "1": "오답 근거: 7은 삭제 대상 35의 왼쪽 부분트리 아래 단말 노드이다. 두 자식 노드 삭제에서 강의가 설명한 후속자는 오른쪽 부분트리의 가장 왼쪽 노드이므로 7은 대체 노드가 아니다.",
    "2": "오답 근거: 30은 35의 왼쪽 자식이다. 이 문항의 삭제 절차는 오른쪽 부분트리에서 후속자를 찾는 방식이므로 30을 35 자리로 바로 올리지 않는다.",
    "3": "정답 근거: 35의 오른쪽 부분트리에서 55→44→40처럼 계속 왼쪽으로 내려가면 가장 작은 키 40을 만난다. 이 중위 후속자 40이 35의 자리를 대신한다.",
    "4": "오답 근거: 55는 오른쪽 부분트리의 루트이지만 그 왼쪽에 44와 40이 있다. 오른쪽 부분트리의 최솟값이 아니므로 후속자로 선택되지 않는다.",
  },
  "2018-1-q30": {
    "1": "오답 근거: 60은 70이 들어갈 오른쪽 리프의 왼쪽 키로 남는다. t=2 B-트리에서 포화 리프 [60, 80, 90]을 나눌 때 부모로 올라가는 가운데 키는 80이다.",
    "2": "오답 근거: 70은 새로 삽입되는 키이며 분할 뒤 [60] 쪽 리프에 들어간다. 부모로 승격되는 키는 기존 포화 노드의 가운데 키 80이다.",
    "3": "정답 근거: t=2 B-트리는 한 노드가 최대 3개 키를 가지며, [60, 80, 90]을 분할할 때 가운데 키 80을 부모로 보낸다. 그 뒤 70은 왼쪽 새 리프 [60]에 삽입된다.",
    "4": "오답 근거: 90은 분할 뒤 오른쪽 새 리프에 남는 큰 키이다. 부모로 이동하는 가운데 키가 아니므로 70 삽입 과정에서 승격되지 않는다.",
  },
  "2019-1-q16": {
    "1": "오답 근거: 35는 단위 이익이 큰 물체2와 물체4만 모두 넣은 값 15+20이다. 원문은 물체를 쪼갤 수 있다고 했으므로 남은 용량 2에 물체3 일부를 더 넣어야 한다.",
    "2": "오답 근거: 38은 0/1 배낭처럼 물체를 통째로만 고를 때 나올 수 있는 후보값이다. 이 문항은 분할 가능 배낭이라 단위 이익 순서대로 남은 용량만큼 잘라 넣는다.",
    "3": "정답 근거: 단위 이익은 물체2=5, 물체4=4, 물체3=3.5, 물체1=3 순서이다. 물체2와 물체4를 넣고 남은 용량 2에 물체3을 절반 넣으면 15+20+7=42이다.",
    "4": "오답 근거: 49는 용량 10을 넘지 않는 분할 가능 선택으로 만들 수 없다. 물체2와 물체4를 넣은 뒤 남은 용량은 2뿐이므로 추가 가능한 최대 이익은 물체3의 절반 이익 7이다.",
  },
  "2019-1-q18": {
    "1": "오답 근거: 15는 여섯 정점을 모두 연결하는 다섯 간선의 합이 아니다. d-f=4까지 고르면 {a,b,c,e}와 {d,f} 두 성분이 남아 e-f=6 같은 연결 간선이 추가로 필요하다.",
    "2": "정답 근거: 크루스칼 순서로 c-e=1, b-e=2, a-c=3, d-f=4를 선택하고 a-b=5는 사이클이라 제외한 뒤 e-f=6을 더하면 모든 정점이 연결된다. 합은 1+2+3+4+6=16이다.",
    "3": "오답 근거: 17은 마지막 연결 간선으로 e-f=6 대신 더 무거운 a-d=7을 고른 경우의 합이다. 최소 신장 트리는 가능한 연결 간선 중 더 작은 e-f=6을 선택한다.",
    "4": "오답 근거: 18은 사이클 간선이나 더 큰 연결 간선을 포함해 누적한 값이다. MST는 사이클을 만들지 않는 최소 가중치 간선만 남기므로 18까지 커지지 않는다.",
  },
  "2019-1-q19": {
    "1": "오답 근거: t₁=(0,4)는 시작 시간이 가장 빠른 작업이지만 종료 시간은 4이다. 작업 선택 욕심쟁이 방법의 첫 선택 기준은 시작 시간이 아니라 가장 빠른 종료 시간이다.",
    "2": "오답 근거: t₄=(1,6)는 t₆보다 늦게 끝난다. 종료 시간이 6인 작업을 먼저 고르면 이후 선택 가능한 시간 구간을 t₆보다 좁게 남긴다.",
    "3": "정답 근거: 원문 표에서 t₆=(1,3)은 종료 시간이 3으로 가장 작다. 작업 선택 문제의 욕심쟁이 전략은 가장 빨리 끝나는 작업을 먼저 기계에 할당한다.",
    "4": "오답 근거: t₇=(3,8)은 t₆이 끝나는 시각 3에 시작할 수 있는 후속 후보이지만, 첫 배정 작업은 아니다. 첫 선택은 종료 시간이 가장 빠른 t₆이다.",
  },
  "2019-1-q22": {
    "1": "오답 근거: 완전 역순인 50 40 30 20 10의 인접 교환 수는 역전쌍 수와 같다. 다섯 원소의 역전쌍은 4+3+2+1=10개이므로 8회는 두 교환을 빠뜨린 값이다.",
    "2": "정답 근거: 50은 뒤의 네 원소와, 40은 세 원소와, 30은 두 원소와, 20은 한 원소와 순서가 뒤집혀 있다. 역전쌍 합 4+3+2+1=10이 버블 정렬의 총 자리바꿈 횟수이다.",
    "3": "오답 근거: 12는 다섯 원소 사이에 가능한 모든 역전쌍 수 10을 초과한다. 인접 교환은 역전쌍을 하나씩 줄이므로 완전 역순 배열에서도 10회를 넘지 않는다.",
    "4": "오답 근거: 15는 여섯 원소 완전 역순에서나 나오는 조합 수 C(6,2)에 해당한다. 원문 배열은 다섯 원소이므로 최대 역전쌍 수는 C(5,2)=10이다.",
  },
  "2019-1-q35": {
    "1": "오답 근거: 버텍스 커버는 모든 간선이 선택된 정점에 닿도록 정점 집합을 고르는 문제이다. 원문 절차처럼 MST를 만든 뒤 DFS 방문 순서로 순회 경로를 만드는 방식과 다르다.",
    "2": "정답 근거: MST를 만들고 그 트리를 DFS 순서로 나열한 뒤 첫 정점으로 돌아오는 절차는 외판원 문제의 근사 알고리즘 흐름이다. 모든 정점을 방문하고 출발점으로 복귀하는 순회가 핵심이다.",
    "3": "오답 근거: CNF-만족성 문제는 논리식의 각 절을 만족시키는 진리값 배정 존재를 묻는다. 그래프의 MST와 DFS 순회 절차를 사용하지 않는다.",
    "4": "오답 근거: 클리크 판정 문제는 서로 모두 인접한 정점 부분집합의 존재를 묻는다. 원문처럼 신장 트리와 DFS 순서를 이용해 순회 경로를 구성하는 문제가 아니다.",
  },
  "2019-1-q03": {
