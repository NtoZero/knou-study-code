...
function formatQuestion(question: (typeof algorithmPastExamQuestions)[number]) {
  return `${question.year}-${question.number}`;
}

export default function AlgorithmPastExamCoverage({ lectureId }: { lectureId: number }) {
  ...
  return (
    <section className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-5 dark:border-emerald-900 dark:bg-emerald-950/20">
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-gray-600 dark:text-gray-300">
            2017~2019 알고리즘 기출에서 이 강의와 연결된 문항을 기준으로 출제 개념, 풀이 요구, 오답 기준을 확인합니다.
          </p>
        </div>

        <div className="grid grid-cols-3 gap-2 text-center text-xs">
          <div className="rounded-lg border border-emerald-200 bg-white px-3 py-2 dark:border-emerald-900 dark:bg-gray-950">
            <div className="font-mono text-lg font-bold text-emerald-700 dark:text-emerald-200">
              {questions.length}
            </div>
        ...
        <div className="rounded-lg border border-emerald-200 bg-white p-4 dark:border-emerald-900 dark:bg-gray-950">
          ...
          <div className="flex flex-wrap gap-2">
            {questions.map((question) => (
              <Link
                key={question.id}
                href={`/algorithm/past-exam?year=${question.year}#${question.id}`}
                className="rounded-lg bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-700 transition-colors hover:bg-emerald-200 dark:bg-emerald-950 dark:text-emerald-100 dark:hover:bg-emerald-900"
              >
      ...
      <div className="space-y-3">
        {representative.map((item) => (
          <article
            key={item.concept}
            className="rounded-lg border border-emerald-200 bg-white p-4 dark:border-emerald-900 dark:bg-gray-950"
          >
            ...
            <div className="grid gap-3 lg:grid-cols-3">
              ...
              <div>
                <div className="mb-1 flex items-center gap-1 text-xs font-bold uppercase text-emerald-700 dark:text-emerald-200">
